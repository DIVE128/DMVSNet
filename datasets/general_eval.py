...
class MVSDataset(Dataset):
    ...
    def read_img(self, filename):
        ...
        # colorEnhancer = ImageEnhance.Sharpness(img)
        # img = colorEnhancer.enhance(40.0)
        # scale 0~255 to 0~1
    ...
    def scale_mvs_input(self, img, intrinsics, max_w, max_h, base=global_base):
        h, w = img.shape[:2]
    ...
    def scale_depth_input(self,img,max_w, max_h, base=global_base,interp=cv2.INTER_NEAREST):
        h, w = img.shape[:2]
        if h > max_h or w > max_w:
            scale = 1.0 * max_h / h
            if scale * w > max_w:
            ...
            new_w, new_h = 1.0 * w // base * base, 1.0 * h // base * base
        img = cv2.resize(img, (int(new_w), int(new_h)),interpolation=interp)
        return img
