...
def compute_metrics_for_each_image(metric_func):
    def wrapper(depth_est, depth_gt, mask, *args):
        ...
        for idx in range(batch_size):
            ...
            if torch.isnan(ret):
                results.append(torch.zeros_like(ret))
            else:
                results.append(ret)
...
@torch.no_grad()
@compute_metrics_for_each_image
def Thres_metrics(depth_est, depth_gt, mask, thres,return_Mean=False):
    assert isinstance(thres, (int, float))
    depth_est, depth_gt = depth_est[mask], depth_gt[mask]
    errors = torch.abs(depth_est - depth_gt)
    err_mask = errors > thres
    if return_Mean==True:
        return torch.mean(err_mask.float()),errors[~err_mask].mean()
    else:
        if torch.isnan(torch.mean(err_mask.float())):
            return torch.zeros_like(torch.mean(err_mask.float()))
        else:
